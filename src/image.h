...
public:
    ...
    std::span<const float> RowPtr(int row, int channel) const {
        return std::span<const float>(data.data() + (channel * w * h) + (row * w), w);
    }

    std::span<float> RowPtr(int row, int channel) {
        return std::span<float>(data.data() + (channel * w * h) + (row * w), w);
    }
    ...
    bool is_nonempty_patch(int x, int y, int w = 0) const {
        for (int q1 = x - w; q1 <= x + w; q1++)
            for (int q2 = y - w; q2 <= y + w; q2++) {
                int c1 = 0;
...
// Image I/O functions
inline Image load_binary(const string &filename) {
    Image im;
    im.load_binary(filename);
    return im;
}

inline Image load_image(const string &filename) {
    Image im;
